...
// Add this inside your updateImage function to conditionally use the camera image if available.

if (cameraImageLoaded) {
    segmentedPhoto = await loadLocalImage(cameraStream);
} else {
    async function loadImage(file) {
        return new Promise((resolve) => {
            const reader = new FileReader()
...
// Add this to your existing loadImage function to load the local image.
...
// Add this event listener to handle the camera button click.
cameraButton.addEventListener("click", async () => {
    try {
        cameraStream = await navigator.mediaDevices.getUserMedia({ video: true });
        cameraImageLoaded = true;
        const localImage = await loadLocalImage(cameraStream);
        // Display the local image in the resultCanvas or wherever you want.
    } catch (error) {
